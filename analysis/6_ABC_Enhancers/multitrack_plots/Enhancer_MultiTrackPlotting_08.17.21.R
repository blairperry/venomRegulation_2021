...
svmp.vpers <- read_tsv('./analysis/6_ABC_Enhancers/ABC_output/_reformat/SVMP_EnhancerPredictionsFull_VenomGenes_simple_newID_08.16.21.bed',col_names = F) %>% 
...
svmp.fullPlots <- p.svmp.genes  / p.svmp.h3k27ac.chip / p.svmp.h3k4me3.chip/ p.svmp.atac.vg3 + p.svmp.scale + plot_layout(ncol = 1,heights = c(2,2,2,2,1))
svmp.fullPlots
...
svsp.vpers <- read_tsv('./analysis/6_ABC_Enhancers/ABC_output/_reformat/SVSP_EnhancerPredictionsFull_VenomGenes_simple_newID_08.16.21.bed',col_names = F) %>% 
  select(molecule=1,start=2,end=3,id=4) %>%  
  mutate(gene = str_split_fixed(id, '_',2)[,2]) %>% 
  mutate(gene = str_split(gene,'\\.')) %>% 
  unnest(gene) %>% 
...
# svsp.tes.enhan <- read_tsv('../../../../_tes_giulia/CroMit_hAT-Tip_100/various_bed/RegPeaks_hAT-Tip.bed.txt',col_names = F) %>% 
#   filter(str_detect(X4,'PER'))
# 
# svsp.tes.promo <- read_tsv('../../../../_tes_giulia/CroMit_hAT-Tip_100/various_bed/RegPeaks_hAT-Tip.bed.txt',col_names = F) %>% 
#   filter(str_detect(X4,'transcript'))
# 
# svsp.tes.all <- read_tsv('../../../../_tes_giulia/CroMit_hAT-Tip_100/various_bed/CroMit_hAT-Tip100_genomic.bed.txt',col_names = F) %>% 
#   filter(X1 == 'scaffold-mi2') %>% 
#   mutate(type = ifelse(X2 %in% svsp.tes.enhan$X6,'Enhancer','Other')) %>% 
#   mutate(type = ifelse(X2 %in% svsp.tes.promo$X6,'Promoter',type)) %>% 
#   mutate(type = factor(type,levels=c('Other','Enhancer','Promoter')))
# 
# svsp.focalTEs <- svsp.tes.all %>% 
#   filter(X3 >= svsp.reg.start & X2 <= svsp.reg.end)
# 
# write_tsv(svsp.focalTEs,'/Users/perryb/Dropbox/CastoeLabFolder/projects/CVV_Gene_Regulation/_VenomGeneRegulation/analyses/_tes_giulia/CroMit_hAT-Tip_100/various_bed/SVSP_Focal_haT-Tips_bwp.bed',col_names = F)
...
# 
# p.svsp.tes <- ggplot(svsp.tes.all,aes(x=(X2+X3)/2,y=type,color=type)) +
#   geom_point(size=3,alpha=0.8)+
#   scale_x_continuous(labels = comma,limits=c(svsp.reg.start,svsp.reg.end),expand=c(0,0)) +
#   ylab('')+
#   ggtitle('CroMit_hAT-tip100 Locations') +
#   scale_color_manual(values = c('Promoter'='#2C8790','Enhancer'='#82B47A','Other'='grey')) +
#   theme_classic(base_size = 14) +
#   labs(color='Location') +
#   theme(axis.line.y = element_blank(),plot.title.position = 'plot',plot.title = element_text(color='black',face='bold',size = 14),axis.title.x = element_blank())
# p.svsp.tes
...
svsp.fullPlots <- p.svsp.genes / p.svsp.h3k27ac.chip / p.svsp.h3k4me3.chip/ p.svsp.atac.vg3 + p.svsp.scale + plot_layout(ncol = 1,heights = c(2,2,2,2,1))
svsp.fullPlots
...
pla2.vpers <- read_tsv('./analysis/6_ABC_Enhancers/ABC_output/_reformat/PLA2_EnhancerPredictionsFull_VenomGenes_simple_newID_08.16.21.bed',col_names = F) %>% 
  select(molecule=1,start=2,end=3,id=4) %>%  
  mutate(gene = str_split_fixed(id, '_',2)[,2]) %>% 
  mutate(gene = str_split(gene,'\\.')) %>% 
  unnest(gene) %>% 
...
pla2.fullPlots <- p.pla2.genes  / p.pla2.h3k27ac.chip / p.pla2.h3k4me3.chip/ p.pla2.atac.vg3 + p.pla2.scale + plot_layout(ncol = 1,heights = c(2,2,2,2,1))
pla2.fullPlots
