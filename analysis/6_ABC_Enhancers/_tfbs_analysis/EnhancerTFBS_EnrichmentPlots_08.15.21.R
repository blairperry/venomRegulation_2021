...
svmp.enrich <- read_csv('analysis/6_ABC_Enhancers/_tfbs_analysis/SVMP.vs.NonVen/Enrichment: Text2038807373472941107_MostSigDeficit.csv') %>% 
  janitor::clean_names() %>% 
  filter(gene_p_value < 0.05 & gene_representation == 'Up') %>% 
  mutate(family = 'SVMP')

svsp.enrich <- read_csv('analysis/6_ABC_Enhancers/_tfbs_analysis/SVSP.vs.NonVen/Enrichment: Text6985771775548605667_MostSigDeficit.csv') %>% 
  janitor::clean_names() %>% 
  filter(gene_p_value < 0.05 & gene_representation == 'Up')%>% 
  mutate(family = 'SVSP')

pla2.enrich <- read_csv('analysis/6_ABC_Enhancers/_tfbs_analysis/PLA2.vs.NonVen/Enrichment: Text5115956483220433109_MostSigDeficit.csv') %>% 
  janitor::clean_names() %>% 
  filter(gene_p_value < 0.05 & gene_representation == 'Up')%>% 
  mutate(family = 'PLA2')
...
all.boundTFcounts <- svmp.tf.ali %>% 
  ...
  select(family,PER_ID,tfbs_id,avg_footprint,tot.bound,Median1DPE) %>% 
  unique() %>% 
  filter(tot.bound > 1) %>% 
  group_by(family,PER_ID,tfbs_id,Median1DPE) %>% 
  tally() %>% 
  arrange(Median1DPE) %>% 
...
all.exp <- svmp.tf.ali %>% 
  bind_rows(svsp.tf.ali,pla2.tf.ali) %>% 
  # mutate(family = str_split_fixed(gene_id,'[ ]',2)[,1]) %>% 
  select(family,PER_ID,Median1DPE,meanExp) %>% 
  unique() %>% 
  filter(PER_ID %in% all.boundTFcounts$PER_ID) %>% 
  arrange(Median1DPE) %>% 
  mutate(PER_ID = factor(PER_ID,levels=unique(.$PER_ID)))

p1 <- ggplot(all.boundTFcounts,aes(x=tfbs_id,y=PER_ID,size=n,color=tfbs_id)) +
...
p2 <- ggplot(all.exp,aes(x=log10(Median1DPE+1),y=PER_ID,fill=meanExp)) +
