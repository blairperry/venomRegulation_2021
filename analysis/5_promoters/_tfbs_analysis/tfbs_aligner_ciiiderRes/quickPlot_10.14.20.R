...
setwd('/Users/perryb/Dropbox/CastoeLabFolder/projects/CVV_Gene_Regulation/_VenomGeneRegulation/analyses/transcription_factors/_tfbs_analysis/_Promoter_CIIIDER_TFBS_HierarchicalEnrichmentAnalyses_09.29.20/7_HighestExpressed.vs.Paralogs/hSVMP.vs.paralogs')
...
tf.ali <- read_tsv('./SVMPandParalogs_promSeqs.align.fixed_alignTFBS.txt',col_names = F) %>% 
  mutate(tfbs_id = str_split_fixed(X4,'_',2)[,1]) %>% 
  mutate(X1 = str_split_fixed(X1,'[::]',2)[,1])

cons_score <- read_tsv('./SVMPandParalogs_promSeqs.align.fixed_consScore.txt',col_names = F)

gaps <- read_tsv('SVMPandParalogs_promSeqs.align.fixed_gaps.txt',col_names = F) %>% 
  mutate(X1 = str_split_fixed(X1,'[::]',2)[,1])
