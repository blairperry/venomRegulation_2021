...
svmp.tf.ali <- read_tsv('./analysis/5_promoters/_tfbs_analysis/SVMP.vs.NonVen/tfbs_aligned/SVMP_PromPeaks_seqs_08.10.21.aln.mafft.singleLine_alignTFBS.txt',
...
svmp.cons_score <- read_tsv('./analysis/5_promoters/_tfbs_analysis/SVMP.vs.NonVen/tfbs_aligned/SVMP_PromPeaks_seqs_08.10.21.aln.mafft.singleLine_consScore.txt',col_names = c('position','score'))

svmp.gaps <- read_tsv('./analysis/5_promoters/_tfbs_analysis/SVMP.vs.NonVen/tfbs_aligned/SVMP_PromPeaks_seqs_08.10.21.aln.mafft.singleLine_gaps.txt',col_names = F) %>% 
...
svmp.p3 <- ggplot(svmp.tf.ali %>% select(gene_id,avg1DPE) %>% unique(),aes(x=reorder(gene_id,desc(gene_id)),y=avg1DPE)) +
...
svsp.tf.ali <- read_tsv('./analysis/5_promoters/_tfbs_analysis/svsp.vs.NonVen/tfbs_aligned/svsp_PromPeaks_seqs_08.10.21.aln.mafft.singleLine_alignTFBS.txt',
                        col_names = c('PER_ID','start','end','tfbs_split','strand','RVG11_Footprint','RVG1_Footprint','RVG4_Footprint','MA_ID')) %>% 
  mutate(tfbs_id = str_split_fixed(tfbs_split,'_',2)[,1]) %>% 
  mutate(tx_id = str_split_fixed(PER_ID,'[::]',2)[,1]) %>% 
  left_join(exp,by=c('tx_id'='txid')) %>% 
...
svsp.cons_score <- read_tsv('./analysis/5_promoters/_tfbs_analysis/svsp.vs.NonVen/tfbs_aligned/svsp_PromPeaks_seqs_08.10.21.aln.mafft.singleLine_consScore.txt',col_names = c('position','score'))

svsp.gaps <- read_tsv('./analysis/5_promoters/_tfbs_analysis/svsp.vs.NonVen/tfbs_aligned/svsp_PromPeaks_seqs_08.10.21.aln.mafft.singleLine_gaps.txt',col_names = F) %>% 
  mutate(tx_id = str_split_fixed(X1,'[::]',2)[,1]) %>% 
  # mutate(X1 = str_remove(X1,'crovir-transcript-')) %>% 
...
svsp.p3 <- ggplot(svsp.tf.ali %>% select(gene_id,avg1DPE) %>% unique(),aes(x=reorder(gene_id,desc(gene_id)),y=avg1DPE)) +
  geom_bar(stat='identity',fill='firebrick4') +
  coord_flip() +
...
pla2.tf.ali <- read_tsv('./analysis/5_promoters/_tfbs_analysis/pla2.vs.NonVen/tfbs_aligned/pla2_PromPeaks_seqs_08.10.21.aln.mafft.singleLine_alignTFBS.txt',
                        col_names = c('PER_ID','start','end','tfbs_split','strand','RVG11_Footprint','RVG1_Footprint','RVG4_Footprint','MA_ID')) %>% 
  mutate(tfbs_id = str_split_fixed(tfbs_split,'_',2)[,1]) %>% 
  mutate(tx_id = str_split_fixed(PER_ID,'[::]',2)[,1]) %>% 
  left_join(exp,by=c('tx_id'='txid')) %>% 
...
pla2.cons_score <- read_tsv('./analysis/5_promoters/_tfbs_analysis/pla2.vs.NonVen/tfbs_aligned/pla2_PromPeaks_seqs_08.10.21.aln.mafft.singleLine_consScore.txt',col_names = c('position','score'))

pla2.gaps <- read_tsv('./analysis/5_promoters/_tfbs_analysis/pla2.vs.NonVen/tfbs_aligned/pla2_PromPeaks_seqs_08.10.21.aln.mafft.singleLine_gaps.txt',col_names = F) %>% 
  mutate(tx_id = str_split_fixed(X1,'[::]',2)[,1]) %>% 
  # mutate(X1 = str_remove(X1,'crovir-transcript-')) %>% 
...
pla2.p3 <- ggplot(pla2.tf.ali %>% select(gene_id,avg1DPE) %>% unique(),aes(x=reorder(gene_id,desc(gene_id)),y=avg1DPE)) +
  geom_bar(stat='identity',fill='firebrick4') +
  coord_flip() +
